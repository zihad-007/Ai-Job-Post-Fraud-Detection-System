...
def dashboard():
    ...
    total = conn.execute(
        "SELECT COUNT(*) FROM job_posts WHERE user_id=?",
        (session["user_id"],),
    ).fetchone()[0]
    fraud = conn.execute(
    ...
    trend_rows = conn.execute(
        ...
        FROM job_posts
        WHERE user_id=? AND timestamp >= datetime('now', '-30 day')
        GROUP BY day
        ORDER BY day ASC
        """,
        (session["user_id"],),
    ...
    desc_rows = conn.execute(
        "SELECT description, result FROM job_posts WHERE user_id=? AND timestamp >= datetime('now', '-30 day')",
        (session["user_id"],),
    ).fetchall()
    conn.close()
    ...
    real = total - fraud
    accuracy = round((real / total) * 100) if total > 0 else 0
...
def submit_job():
    ...
    if request.method == "POST":
        ...
        if not job_validation.get("is_job", False):
            flash("This does not appear to be a job posting.", "error")
            return render_template(
                "result.html",
                result="Not a Job Post",
                job_text=job_text,
                details=job_validation,
            )
